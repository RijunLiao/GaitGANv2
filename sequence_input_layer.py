...
def processImageCrop(im_info, transformer, flow):
  ...
  im_flip = im_info[3]
  data_in_connect = caffe.io.load_image(im_path) # 0-1 scale

  #data_in = caffe.io.resize_image(data_in_connect[:,256:,:], (64,64))  # 0-1 scale right 
  #data_in = cv2.resize(data_in_connect[:,256:,0], (64,64))
  data_in = data_in_connect[:,64:,0]
  #data_in = data_in*2.0-1.0
  #data_in[data_in>=0.5]=1
  #data_in[data_in<0.5]=0
  #data_in_90 = caffe.io.resize_image(data_in_connect[:,0:256:,:], (64,64))  # 0-1 scale left
  #data_in_90 = cv2.resize(data_in_connect[:,0:256:,0], (64,64))  # 0-1 scale left
  data_in_90 = data_in_connect[:,0:64:,0]
  #data_in_90 = data_in_90*2.0-1
  #data_in_90[data_in_90>=0.5]=1
  #data_in_90[data_in_90<0.5]=0
  #cv2.imshow("face_rot", data_in)
  #cv2.waitKey(0) 
  #raw_input('sdf')
  #print data_in[32,:,:]
  #data_in = data_in[64:128,:,:]
  '''
  clip_H = 10 + 20
  shift_H = 10 + 20/2
  save_img_size=64
  data_in = np.zeros((save_img_size,save_img_size,1))
  data_in[shift_H:save_img_size-(clip_H-shift_H),:,:] = data_in_temp[clip_H:save_img_size,:,:]
  '''
  processed_image_temp = np.zeros((64,128,1),dtype='float')
  processed_image_temp[:,0:64,0] = data_in # right image not 90 degree
  processed_image_temp[:,64:128,0] = data_in_90  # left image 90 => label_map
